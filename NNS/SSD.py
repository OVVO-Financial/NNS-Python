...
def NNS_SSD(x: pd.Series, y: pd.Series) -> str:
    r"""
    NNS SSD Test
    ...
    if (not x_ssd_y) and (x.min() >= y.min()) and (x.mean() >= y.mean()) and (not LPM_x_sort.equals(LPM_y_sort)):
        return "X SSD Y"
    if (not y_ssd_x) and (y.min() >= x.min()) and (y.mean() >= x.mean()) and (not LPM_x_sort.equals(LPM_y_sort)):
        return "Y SSD X"
